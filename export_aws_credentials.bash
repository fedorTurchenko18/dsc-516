...
EXPORT_DOTENV_SCRIPT="export_dotenv.bash"
...
if [ ! -f "$PEM_KEY_PATH/$CREDENTIALS_FILE" ]; then
    cd "$(pwd)"
    aws configure list
    aws ec2 create-key-pair --key-name FlowerKey --query 'KeyMaterial' --output text > ./aws_management/$AWS_KEY_PAIR.pem
    chmod 400 ./aws_management/$AWS_KEY_PAIR.pem
fi